function generateText(){
	...
	readGR2Info(desk).done(function(info){
		...
		//WSDE and EF_RANGE REMOVED BY MM on 5/7/20 per NWS HQ guidance
		//summaryTxt = summaryTxt + ' with potential peak winds of ' + text['WSDE']  + ' (' + text['EF_RANGE'] + ') is ' + text['CONFIDENCE_A'] + '.';
		summaryTxt = summaryTxt + ' is ' + text['CONFIDENCE_A'] + '.';
		...
		if(follow == 'yes'){
			...
			checkTextArea("textarea-1", summaryTxt2 + txtv1 + txt3);
			jQuery("#textarea-2").val(summaryTxt2 + txtv2 + txt3);
			checkTextArea("textarea-2", summaryTxt2 + txtv2 + txt3);
		}else{
			jQuery("#textarea-1").val(summaryTxt + txtv1 + txt3);
			checkTextArea("textarea-1", summaryTxt + txtv1 + txt3)
			jQuery("#textarea-2").val(summaryTxt + txtv2 + txt3);
			checkTextArea("textarea-2", summaryTxt + txtv2 + txt3);
		}	
	});		
}
...
function checkTextArea(id, text){
		...
		if(which === "1"){
			...
			jQuery("#errorcheck2").empty();
			jQuery("#errorcheck2").html(out);
		}	
}
